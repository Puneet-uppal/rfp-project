  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <button onClick={() => navigate('/rfps')} className="text-violet-950 hover:underline mb-2">
            ← Back to RFPs
          </button>
          <h1 className="text-2xl font-bold text-violet-950">{rfp.title}</h1>
          <span className={`inline-block mt-2 px-3 py-1.5 text-xs font-semibold rounded-lg border-2 uppercase tracking-wide ${
            rfp.status === RfpStatus.DRAFT ? 'bg-violet-100 text-violet-950 border-violet-300' :
            rfp.status === RfpStatus.SENT ? 'bg-violet-200 text-violet-950 border-violet-400' :
            rfp.status === RfpStatus.EVALUATING ? 'bg-violet-50 text-violet-950 border-violet-400' :
            rfp.status === RfpStatus.AWARDED ? 'bg-violet-400 text-white border-violet-600' :
            rfp.status === RfpStatus.CLOSED ? 'bg-violet-500 text-white border-violet-700' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            {rfp.aiSummary && (
              <div className="mt-5 p-4 bg-gradient-to-r from-violet-50 to-violet-100 rounded-lg border border-violet-100">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-violet-950 leading-relaxed">{rfp.aiSummary}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              {rfp.items.map((item, index) => (
                <div key={index} className="group border border-gray-100 rounded-lg p-5 bg-gradient-to-br from-gray-50 to-white hover:border-violet-200 hover:shadow-md transition-all duration-200">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 space-y-3">
                      <div className="flex items-center gap-3">
                        <span className="flex items-center justify-center w-8 h-8 rounded-full bg-violet-100 text-violet-950 font-semibold text-sm">
                          {index + 1}
                        </span>
                        ...
                        <div className="pl-11 pt-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {Object.entries(item.specifications).map(([key, value]) => (
                              <span key={key} className="inline-flex items-center text-sm bg-violet-50 text-violet-950 px-3 py-1.5 rounded-full border border-violet-100">
                  ...
                  <div key={proposal.id} className="p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg border border-gray-100 hover:border-teal-200 hover:shadow-sm transition-all duration-200">
                    <div className="flex justify-between items-start">
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-3 mt-2">
                            ...
                            {proposal.deliveryDays && (
                              <span className="inline-flex items-center text-sm bg-violet-50 text-violet-950 px-2.5 py-1 rounded-md">
                                <svg className="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <span className={`px-3 py-1.5 text-xs font-medium rounded-full ${
                        ...
                        proposal.status === ProposalStatus.EVALUATED ? 'bg-violet-100 text-violet-950 border border-violet-200' :
                        'bg-gray-100 text-gray-600 border border-gray-200'
                      }`}>
                        {proposal.status}
                      </span>
            ...
            <dl className="space-y-4">
              {rfp.budget && (
                <div className="p-3 bg-gradient-to-r from-emerald-50 to-green-50 rounded-lg border border-emerald-100">
                  ...
                  <dd className="font-bold text-emerald-700 text-xl">{rfp.currency || ''} {rfp.budget.toLocaleString()}</dd>
                </div>
              )}
              {rfp.deliveryDays && (
                <div className="p-3 bg-gradient-to-r from-violet-50 to-violet-100 rounded-lg border border-violet-100">
                  <dt className="text-xs font-medium text-violet-950 uppercase tracking-wide mb-1">Delivery Timeline</dt>
                  <dd className="font-semibold text-violet-950">Within {rfp.deliveryDays} days</dd>
                </div>
              )}
              {rfp.paymentTerms && (
