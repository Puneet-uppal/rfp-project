      ...
      {!parsedRfp ? (
        <>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <div className={`relative rounded-xl ${isListening ? 'p-[2px] bg-gradient-to-r from-blue-500 via-blue-300 to-blue-500 bg-[length:200%_100%] animate-shimmer' : ''}`}>
                  <textarea
                    ...
                    rows={6}
                    className={`w-full px-4 py-3 pr-14 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-violet-500 transition-all duration-200 resize-none ${
                      isListening 
                ...
                <button
                  ...
                  className={`absolute right-4 bottom-4 p-2.5 rounded-full transition-all duration-300 z-10 ${
                    isListening
                      ? 'bg-violet-500 text-white shadow-lg shadow-blue-300'
              ...
              {isListening && (
                <div className="mt-2 flex items-center gap-2 text-violet-950">
            ...
            <button
              ...
              className="bg-violet-950 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-violet-900 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200 flex items-center gap-2"
            >
              {isProcessing ? (
                <>
            ...
            <div className="space-y-3">
              {examplePrompts.map((prompt, index) => (
                <button
                  ...
                  className="block w-full text-left p-4 text-sm text-gray-600 bg-gradient-to-r from-gray-50 to-white rounded-xl border border-gray-200 hover:border-violet-300 hover:bg-violet-50/30 hover:text-gray-800 transition-all duration-200 group"
                >
                  <span className="flex items-start gap-3">
                    <span className="flex-shrink-0 w-6 h-6 rounded-full bg-gray-100 group-hover:bg-violet-100 flex items-center justify-center text-gray-400 group-hover:text-violet-950 transition-colors">
                      {index + 1}
                    </span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-6 shadow-sm">
            ...
            {parsedRfp.aiSummary && (
              <div className="bg-gradient-to-r from-violet-50 to-violet-100 p-4 rounded-lg border border-violet-100">
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 w-8 h-8 bg-violet-100 rounded-full flex items-center justify-center">
                    <svg className="w-4 h-4 text-violet-950" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-violet-950 uppercase tracking-wide mb-1">AI Summary</p>
                    <p className="text-sm text-violet-950 leading-relaxed">{parsedRfp.aiSummary}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-5 border-t">
              ...
              {parsedRfp.deliveryDays && (
                <div className="p-3 bg-gradient-to-br from-violet-50 to-violet-100 rounded-lg border border-violet-100">
                  <p className="text-xs font-medium text-violet-950 uppercase tracking-wide mb-1">Delivery</p>
                  <p className="font-semibold text-violet-950">{parsedRfp.deliveryDays} days</p>
                </div>
              )}
            ...
            <div className="pt-5 border-t">
              <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
                <svg className="w-5 h-5 text-violet-950" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="space-y-3">
                {parsedRfp.items.map((item, index) => (
                  <div key={index} className="group border border-gray-100 rounded-lg p-4 bg-gradient-to-br from-gray-50 to-white hover:border-violet-200 hover:shadow-sm transition-all duration-200">
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex items-start gap-3">
                        <span className="flex items-center justify-center w-7 h-7 rounded-full bg-violet-100 text-violet-950 font-semibold text-sm flex-shrink-0">
                          {index + 1}
                        </span>
                        <div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-violet-950 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-violet-900 transition-colors duration-200 flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
