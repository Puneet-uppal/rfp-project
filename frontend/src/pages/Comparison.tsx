  ...
  if (!comparison.proposals || comparison.proposals.length === 0) {
    return (
      <div className="space-y-6">
        <button onClick={() => navigate(`/rfps/${id}`)} className="text-primary-900 hover:underline">
          ← Back to RFP
        </button>
        <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <button onClick={() => navigate(`/rfps/${id}`)} className="text-primary-900 hover:underline mb-2">
            ← Back to RFP
          </button>
        ...
        <button
          ...
          className="bg-primary-900 text-white px-4 py-2 rounded-md hover:bg-primary-950 disabled:opacity-50"
        >
      ...
      {comparison.aiRecommendation && (
        <div className="bg-primary-50 border border-primary-200 rounded-lg p-6">
          ...
          <p className="text-primary-900 mb-4">{comparison.aiRecommendation.reasoning}</p>
                ...
                <tr key={proposal.id} className={isRecommended ? 'bg-slate-50' : ''}>
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        {isRecommended && <span className="text-xs text-primary-900">Recommended</span>}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    ...
                    {comparison.rfp.budget && proposal.totalPrice && (() => {
                      ...
                      return (
                        <p className={`text-xs ${isWithinBudget ? 'text-primary-900' : 'text-red-600'}`}>
                          {displayText}
                        </p>
                      );
                    })()}
                  ...
                  <td className="px-6 py-4">
                    ...
                    {comparison.rfp.deliveryDays && proposal.deliveryDays && (
                      <p className={`text-xs ${Number(proposal.deliveryDays) <= Number(comparison.rfp.deliveryDays) ? 'text-primary-900' : 'text-red-600'}`}>
                  ...
                  <td className="px-6 py-4">
                    {proposal.score ? (
                      <span className={`font-bold text-lg ${
                        Number(proposal.score) >= 80 ? 'text-primary-900' :
                  ...
                  <td className="px-6 py-4">
                    <button
                      ...
                      className="text-primary-900 hover:underline text-sm"
                    >
                      Select Deal
                    </button>
                  </td>
        ...
        {comparison.proposals.map((proposal) => (
          <div key={proposal.id} className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            {proposal.strengths && proposal.strengths.length > 0 && (
              <div className="mb-4">
                <h4 className="text-sm font-medium text-primary-900 mb-2">✓ Strengths</h4>
          ...
          <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-xl">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2.5 bg-primary-900 text-white rounded-lg font-medium hover:bg-primary-950 transition-colors"
              >
                Confirm Deal
              </button>
            </div>
